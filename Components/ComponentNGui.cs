	...
	public class ComponentNGui : ComponentGui, IUpdateable
	{
		...
		public new void HandleInput()
		{
			...
			if (m_sneakButtonWidget.IsClicked || playerInput.ToggleSneak)
			{
				...
				{
					DisplaySmallMessage(Utils.Get(m_componentPlayer.ComponentBody.IsSneaking ? "潜行模式：开" : "潜行模式：关"),Color.White, false, false);
				}
			}
			if (componentRider != null && (m_mountButtonWidget.IsClicked || playerInput.ToggleMount))
			{
				...
				{
					DisplaySmallMessage(Utils.Get(componentRider.Mount != null ? "上马" : "下马"), Color.White, false, false);
				}
			}
			...
			if (isCreative && (m_creativeFlyButtonWidget.IsClicked || playerInput.ToggleCreativeFly) && componentRider.Mount == null)
			{
				...
				if (m_componentPlayer.ComponentLocomotion.IsCreativeFlyEnabled != isCreativeFlyEnabled)
				{
					if (m_componentPlayer.ComponentLocomotion.IsCreativeFlyEnabled)
					{
						...
						DisplaySmallMessage(Utils.Get("飞行模式：开"),Color.White, false, false);
					}
					else
						DisplaySmallMessage(Utils.Get("飞行模式：关"),Color.White, false, false);
				}
			}
			if (m_cameraButtonWidget.IsClicked || playerInput.SwitchCameraMode)
			{
				...
				if (gameWidget.ActiveCamera.GetType() == typeof(FppCamera))
				{
					...
					DisplaySmallMessage(Utils.Get("第三人称视角"),Color.White, false, false);
				}
				else if (gameWidget.ActiveCamera.GetType() == typeof(TppCamera))
				{
                    gameWidget.ActiveCamera = gameWidget.FindCamera<OrbitCamera>(true);
					DisplaySmallMessage(Utils.Get("滑轨视角"),Color.White, false, false);
				}
				...
				else if (gameWidget.ActiveCamera.GetType() == typeof(RandomJumpCamera))
				{
					...
					DisplaySmallMessage(Utils.Get("垂直飞行视角"),Color.White, false, false);
				}
				else
				{
                    gameWidget.ActiveCamera = gameWidget.FindCamera<FppCamera>(true);
					DisplaySmallMessage(Utils.Get("第一人称视角"),Color.White, false, false);
				}
			}
			if (m_photoButtonWidget.IsClicked || playerInput.TakeScreenshot)
			{
				ScreenCaptureManager.CapturePhoto(
                    () => {
                        ...
                        {
                            DisplaySmallMessage(Utils.Get("照片已保存到图片相册"), Color.White,false, false);
                        });
                    },
                    Log.Error
                );
			...
			if (isCreative && (m_timeOfDayButtonWidget.IsClicked || playerInput.TimeOfDay))
			{
				...
				if (num6 == num10)
				{
					...
					DisplaySmallMessage(Utils.Get("黎明"),Color.White, false, false);
				}
				else if (num7 == num10)
				{
					m_subsystemTimeOfDay.TimeOfDayOffset += num7;
					DisplaySmallMessage(Utils.Get("中午"), Color.White, false, false);
				}
				else if (num8 == num10)
				{
					m_subsystemTimeOfDay.TimeOfDayOffset += num8;
					DisplaySmallMessage(Utils.Get("黄昏"), Color.White, false, false);
				}
				else if (num9 == num10)
				{
					m_subsystemTimeOfDay.TimeOfDayOffset += num9;
					DisplaySmallMessage(Utils.Get("午夜"), Color.White, false, false);
				}
			}
